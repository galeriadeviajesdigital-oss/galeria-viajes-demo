...
async function inicializarSitio() {
    configurarBuscador();
    configurarFormularioCotizacion();
...
function configurarFormularioCotizacion() {
    ...
    const open = () => {
        modal.hidden = false;
        cargarOpcionesFormularioCotizacion();
        const viaje = window.viajeSeleccionadoParaCotizacion;
        if (viaje) {
            const select = document.getElementById("quoteViaje");
            if (select) select.value = viaje.id;
        }
        document.getElementById("quoteNombre")?.focus();
    };
    ...
    window.galeriaQuoteModal = { open, close };
}

function cargarOpcionesFormularioCotizacion() {
    const destinos = window.catalogoDestinos || [];
    const experiencias = window.catalogoExperiencias || [];
    const viajes = window.catalogoViajes || [];
    llenarSelectCotizacion("quoteDestino", destinos.map(x => ({id:x.id,nombre:x.nombre})), "Seleccionar destino...");
    llenarSelectCotizacion("quoteExperiencia", experiencias.map(x => ({id:x.id,nombre:x.nombre})), "Seleccionar experiencia...");
    llenarSelectCotizacion("quoteViaje", viajes.map(x => ({id:x.id,nombre:x.nombre})), "Seleccionar viaje...");
}

function llenarSelectCotizacion(id, items, placeholder) {
    const select=document.getElementById(id); if(!select)return;
    const current=select.value;
    select.innerHTML=`<option value="">${placeholder}</option>` + items.map(x=>`<option value="${escaparHTML(x.id)}">${escaparHTML(x.nombre)}</option>`).join("");
    if(items.some(x=>x.id===current)) select.value=current;
}

async function enviarSolicitudCotizacion(closeModal) {
    const button=document.getElementById("publicQuoteSubmit");
    const message=document.getElementById("publicQuoteMessage");
    const val=id=>document.getElementById(id)?.value?.trim() || null;
    const number=id=>{const v=document.getElementById(id)?.value; return v===""||v==null?null:Number(v);};
    const nombre=val("quoteNombre"); const email=val("quoteEmail");
    if(!nombre || !email){ if(message){message.textContent="Nombre y correo electrónico son obligatorios.";message.className="public-quote-message error";} return; }
    const adultos=number("quoteAdultos") || 1, ninos=number("quoteNinos") || 0;
    const payload={
        p_nombre:nombre,p_apellido:val("quoteApellido"),p_email:email,p_telefono:val("quoteTelefono"),p_whatsapp:val("quoteWhatsapp"),
        p_destino_id:val("quoteDestino"),p_experiencia_id:val("quoteExperiencia"),p_viaje_id:val("quoteViaje"),
        p_fecha_salida:val("quoteFechaSalida"),p_fecha_regreso:val("quoteFechaRegreso"),p_adultos:adultos,p_ninos:ninos,
        p_presupuesto_min:number("quotePresupuestoMin"),p_presupuesto_max:number("quotePresupuestoMax"),p_moneda:val("quoteMoneda")||"USD",
        p_comentarios:val("quoteComentarios"),p_acepta_comunicaciones:document.getElementById("quoteAcepta")?.checked===true
    };
    if(button){button.disabled=true;button.textContent="Enviando...";}
    if(message){message.textContent="";message.className="public-quote-message";}
    const {data,error}=await supabase.rpc("crear_lead_web",payload);
    if(error){
        console.error("[Supabase] Error enviando solicitud:",error);
        if(message){message.textContent="No pudimos enviar tu solicitud. Inténtalo nuevamente.";message.className="public-quote-message error";}
        if(button){button.disabled=false;button.textContent="Enviar solicitud";} return;
    }
    if(message){message.textContent="¡Solicitud enviada! Nuestro equipo se pondrá en contacto contigo.";message.className="public-quote-message success";}
    formResetPublicQuote();
    setTimeout(closeModal,1800);
}
function formResetPublicQuote(){document.getElementById("publicQuoteForm")?.reset();}
